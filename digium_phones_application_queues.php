...
$('form').submit(function() {
	$('#managers').attr("multiple", "multiple");
...
/*
	...
	foreach ($devices as $deviceid=>$device) {
			echo '<li id="devices_' . $device['id'] . '">' . $device['name'] . '</li>';
	}
	echo '</ul>';
	echo '</div>';
	...
	echo '<ul id="managers" class="devices ui-menu ui-widget ui-widget-content ui-corner-all ui-sortable">';
...
?>	
	...
	<table>
	<tr>
	<th><a href="#" class="info">Available Devices<span>Displays a listing of devices.</span></a></th>
	<td></td>
	<th><a href="#" class="info">Managers<span>Managers are devices that have permission to view queue details, but are not themselves members of the queue.</span></a></th>
	</tr>
	<tr>
	<td>
		<select size="8" id="devices" style="width: 200px; " ondblclick="addManager($('#devices').val());">
<?php
		foreach ($devices as $deviceid=>$device) {
?>
			<option value="<?php echo $device['id']?>"><?php echo $device['id']?> | <?php echo $device['name']?></option>
<?php
		}
?>
		</select>
	...
	<td align="center">
		<img alt="Remove" src="images/resultset_left.png" onclick="delManager($('#managers').val());" style="width: 24px; height: 24px; "/>
		<img alt="Add" src="images/resultset_right.png" onclick="addManager($('#devices').val());" style="width: 24px; height: 24px; " />
	</td>
	<td>
		<select size="8" id="managers" name="managers[]" style="width: 200px; " ondblclick="delManager($('#managers').val());">
		</select>
	</td>
	</tr>
	</table>
	<h3>HELLLOOOOOO</h3>
<input name="editqueue_submit" value="Save"/>
